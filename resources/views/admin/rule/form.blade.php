...
<form class="form-card" method="POST" action="{{ $mode === 'create' ? route('admin.rule.store') : route('admin.rule.update', $rule) }}">
    ...
    <div class="form-group"><label class="label">Kerusakan</label><select class="select" name="kerusakan_id" required><option value="">Pilih kerusakan</option>@foreach($kerusakan as $k)<option value="{{ $k->id }}" @selected(old('kerusakan_id', $rule->kerusakan_id)==$k->id)>{{ $k->kode_kerusakan }} - {{ $k->nama_kerusakan }}</option>@endforeach</select></div>
    <div class="form-group"><label class="label">Gejala</label><select class="select" name="gejala_id" required><option value="">Pilih gejala</option>@foreach($gejala as $g)<option value="{{ $g->id }}" @selected(old('gejala_id', $rule->gejala_id)==$g->id)>{{ $g->kode_gejala }} - {{ $g->nama_gejala }}</option>@endforeach</select></div>
